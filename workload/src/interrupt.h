...
typedef enum {
    ...
    INTERRUPT_SPI_FPGA_PWM_UNDERFLOW = XPS_FPGA0_INT_ID,
    INTERRUPT_SPI_FPGA_PWM_OVERFLOW = XPS_FPGA1_INT_ID,
...
typedef enum {
    INTERRUPT_CPU0_SGI_IPC_GENERAL = 15,
    INTERRUPT_CPU0_SGI_IPC_WTCP = 14,
    INTERRUPT_CPU0_SGI_IPC_PDP = 13,
    INTERRUPT_CPU0_SGI_IPC_PROCESSDATALINK = 12
} InterruptCpu0SGI_t;
...
typedef enum {
    ...
    INTERRUPT_PRIORITY_T2 = 20 << INTERRUPT_PRIORITY_SHIFT,

    /* when timer is used to provide T0 scheduling, timer interrupt has this priority */
    INTERRUPT_PRIORITY_SCHEDULER_TIMER = 0,

    INTERRUPT_PRIORITY_IPC_TUNNEL_GENERAL = 3 << INTERRUPT_PRIORITY_SHIFT,

    INTERRUPT_PRIORITY_WATCHDOG_DEBUG = 0 << INTERRUPT_PRIORITY_SHIFT
